...
results = LogisticRegression(random_state=0).fit(Xtrain, ytrain)
results.coef_
```
根据系数可以得到logistic回归的表达式为：
$$ln\frac{r}{1-r}=0.056gravity-0.297ph+0.051osmo-0.858cond-0.056urea+0.811calc$$

### 模型的准确率
```{python}
results.score(Xtest, ytest) #测试集准确率
results.score(Xtrain, ytrain) #训练集准确率
cross_val_score(results, Xtrain, ytrain, cv=10).mean()#交叉验证分数
```
根据上面的代码，可以得到模型在测试集上的准确率为79.1%,
在训练集上的准确率为79.2%，两者相差不大，但实际上交叉验证的分数只有73.6%,拟合的只是一般，接下来的目标是选择合适的超参数来提升模型的准确率。
...
for i in C:
  ...
  score = cross_val_score(logit, Xtest, ytest, cv=10).mean()
  test.append(score)
...
根据上面的学习曲线，最优的C值应该在0.01到1.51之间，所以继续细化学习曲线，选择最优C值
```{python}
C = np.linspace(0.01, 1.51, 100)
test = []
...
plt.show()
```
根据打印的结果可以得出细化学习曲线后，交叉验证分数最高为75%，此时的C=0.0706060606060606，比什么都不调提高了1.4%，在调整了重要的正则化参数C以后，还只有这样的准确率，说明逻辑回归拟合这个模型的上限也差不多了，如果需要达到更高的话，需要换其他的分类模型，或者在特征上面做点功夫。
