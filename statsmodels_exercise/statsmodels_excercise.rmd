...
1. 从R中提取数据，建立数据框，提取GDP变量形成Y向量，提取Labor,Kapital,
Technology,Energy四个变量加上截距向量构成设计矩阵X。
...
Y = gdp['GDP']
X_ = gdp.loc[:,['Labor','Kapital','Technology','Energy']]
...
```
根据上面的拟合结果，调整后的R方达到了99.6%，可以说拟合的效果十分的好，
然后观察各个参数的p值，可以发现除了变量Labor和常数外，另外三个参数的
p值都通过了显著性检验，可以初步写出线性回归模型如下：
...
xtx = np.dot(x.T, x)
xty = np.dot(x.T, y)
beta = np.linalg.solve(xtx, xty)#系数矩阵
#results.params 

yhat = np.dot(x, beta)#因变量估计值
#results.predict(x)
...
#results.fvalue
F_crit = stats.f.isf(0.95, p-1, n-p)
p_value = stats.f.sf(F_value, p-1, n-p)
#results.f_pvalue
```
...
```{python}
sigma2hat = sse / (n-p) #求σ^2的估计值
inv = np.linalg.inv(xtx)
cov = sigma2hat * inv#回归系数的方差协方差阵
se = np.sqrt(np.diag(cov)).reshape(-1, 1)#标准误
...
```
上表的结果与之前进行拟合的结果几乎一致，常数项和变量Labor没有通过t检验，
...
u = beta - t_crit * se
c = beta + t_crit * se
[*zip(index, zip(u.ravel(), c.ravel()))]
```
95%的置信区间如上所示,与summary()的置信区间值相差不大。
